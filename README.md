...
MVP-arkitekturen bevarer CoachClips lette, intuitive flow, IndexedDB-lagring og stærke FFmpeg-motor, samtidig med at den garanterer 100% visuel ensartethed mellem web-previewet og det færdige, eksporterede videoklip.
...
For at foretage en ren installation af alle nødvendige pakker:
```bash
npm ci
```
...
1. **Linter (`eslint`)**: Sikrer streng kodekvalitet og syntaksmæssig præcision.
2. **Typecheck (`tsc`)**: Garanterer fuld type-sikkerhed på tværs af frontend og backend.
3. **Enhedstests (`vitest`)**: Tester de underliggende matematiske formler og geometri-hjælpere.
4. **Produktions-Build**: Validerer at applikationen kan kompileres uden fejl.
5. **9x Fuldautomatiserede Smoke-tests**:
   - **`smoke:export:simple`**: Simpel trimning og transkodning.
   - **`smoke:export:no-audio`**: Transkodning af videoer uden lydspor uden nedbrud.
   - **`smoke:export`**: Fuld transkodning med alle markeringstyper (tekst, cirkel, pil, freeze).
   ...
   - **`smoke:export:freeze`**: Tidsmæssig validering og frame-freeze præcision.
   - **`smoke:export:annotations`**: Validering af kombineret HD-transkodning (1280x720).
   - **`smoke:api:simple`**: Gennemtestning af det faktiske REST API (POST, statuspolling og download).

---

## 🎯 Visuel Ensartethed (Preview vs. Eksport)

For at sikre, at det træneren ser på skærmen er **nøjagtig** det samme som det, der eksporteres i videoen, deler previewet og eksport-servicen en centraliseret geometrimotor (`/shared/annotationGeometry.ts`):
...
Vores smoke-tests bruger et avanceret, pixel-baseret regressionsbibliotek (`pixelmatch`), som udtrækker rå videoframer via FFmpeg og sammenligner dem direkte for at garantere visuel overensstemmelse.
...
4. **Sekventiel Jobbehandling**: Serveren behandler eksportjobs i en simpel, stabil køstruktur. Under tæt pilotbrug kan der opleves kortvarig køtid ved tunge eksport-anmodninger.
